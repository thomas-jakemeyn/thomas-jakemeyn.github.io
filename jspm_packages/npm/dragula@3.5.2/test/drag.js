...
test('drag event gets emitted when clicking an item', function(t) {
  ...
  testCase('works when clicking buttons by default', {which: 0}, {
    tag: 'button',
    passes: true
  });
  testCase('works when clicking anchors by default', {which: 0}, {
    tag: 'a',
    passes: true
  });
  testCase('fails for right clicks', {which: 2}, {passes: false});
  testCase('fails for meta-clicks', {
    which: 0,
    metaKey: true
  }, {passes: false});
  testCase('fails for ctrl-clicks', {
    which: 0,
    ctrlKey: true
  }, {passes: false});
  testCase('fails when clicking containers', {which: 0}, {
    containerClick: true,
    passes: false
  });
  testCase('fails whenever invalid returns true', {which: 0}, {
    passes: false,
    dragulaOpts: {invalid: always}
  });
  testCase('fails whenever moves returns false', {which: 0}, {
    passes: false,
    dragulaOpts: {moves: never}
  });
  t.end();
...
test('when dragging, mirror element gets appended to configured mirrorContainer', function(t) {
  ...
  var div = document.createElement('div');
  var item = document.createElement('div');
  var drake = dragula([div], {'mirrorContainer': mirrorContainer});
  item.innerHTML = '<em>the force is <strong>with this one</strong></em>';
  div.appendChild(item);
  document.body.appendChild(div);
  drake.on('cloned', cloned);
...
test('when drag begins, check for copy option', function(t) {
  ...
  var drake = dragula([div], {copy: checkCondition});
  item.innerHTML = '<em>the force is <strong>with this one</strong></em>';
  div.appendChild(item);
  document.body.appendChild(div);
  events.raise(item, 'mousedown', {which: 0});
